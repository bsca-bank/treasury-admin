...
from credit.forms import InvestmentForm, CreditForm
from credit.house import House
from credit.investment import Investment
from credit.mortgage import Mortgage


class AboutView(View):
	"""Return About and Methodology page."""
	template_name = 'credit/about.html'
...
class IndexView(View):
	...
	def get(self, request, *args, **kwargs):
		...
		"""
		float_parameters = ['principal','interest_rate','tax_rate']
		context_dict = {}
		...
		int_parameters = ['nb_year', 'nb_payments_year','grace_period_in_month','amortization_type']
...
class CreditView(View):
	...
	@staticmethod
	def _build_credit(scenario):
		credit = Credit(
			scenario['principal'],
			scenario['interest_rate'],
			scenario['nb_year'],
			scenario['nb_payments_year'],
			scenario['grace_period_in_month'],
			scenario['amortization_type'],
			scenario['tax_rate'],
		)
		return credit
	...
	def get(self, request, *args, **kwargs):
		...
		Returns:
			JsonResponse: Dict containing IRRs, cash stream, and base yearly
				treasury-engine payment
		"""
		form = CreditForm(request.GET)
		if form.is_valid():
			standard_credit = {
				'principal': form.cleaned_data['principal'],
				'interest_rate': form.cleaned_data['interest_rate'],
				'nb_year': form.cleaned_data['nb_year'],
				'nb_payments_year': form.cleaned_data['nb_payments_year'],
				'grace_period_in_month': form.cleaned_data['grace_period_in_month'],
				'amortization_type': form.cleaned_data['amortization_type'],
				'tax_rate': form.cleaned_data['tax_rate']
			}

			# Base stream
			credit = self._build_credit(standard_credit)
			amortization_schedule = credit.get_amortization_schedule()
			context_dict = {
